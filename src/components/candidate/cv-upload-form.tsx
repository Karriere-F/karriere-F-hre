...
export function CvUploadForm({ currentPath }: { currentPath: string | null }) {
  ...
  const onSubmit = (formData: FormData) => {
    ...
    startTransition(async () => {
      ...
      setMessage(result.ok ? "CV envoyé avec succès." : `Erreur : ${result.error}`);
    });
  };
    ...
    <div className="max-w-md">
      {currentPath && (
        <p className="text-sm text-brand-ink-secondary mb-4">
          Un CV est déjà enregistré. Envoyez un nouveau fichier pour le remplacer.
        </p>
      )}

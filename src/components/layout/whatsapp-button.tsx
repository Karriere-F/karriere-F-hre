...
export function WhatsAppButton() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2">
      {open && (
        <div className="mb-1 rounded-lg bg-brand-white shadow-lg border border-brand-grid p-3 flex flex-col gap-2 min-w-[180px]">
          {WHATSAPP_NUMBERS.map((n) => (
            <a
              ...
              className="text-sm text-brand-ink hover:text-brand-good transition-colors"
            >
      ...
      <button
        ...
        onClick={() => setOpen((v) => !v)}
        aria-label="WhatsApp"
        className="h-14 w-14 rounded-full bg-brand-good text-brand-white shadow-lg flex items-center justify-center text-2xl hover:scale-105 transition-transform"
      >

...
import { INDUSTRIE_FICHES, INDUSTRIE_UI, tr } from "@/lib/industrie-metiers";
...
export async function generateMetadata({
  ...
  const fiche = INDUSTRIE_FICHES[slug];
  if (!fiche) return {};
  return buildMetadata({
    pathname: { pathname: "/metiers/industrie/[slug]", params: { slug } },
    locale,
...
export default async function IndustrieFichePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const imageExists =
    !!fiche.image &&
    existsSync(join(process.cwd(), "public", "images", "metiers", "industrie", `${fiche.image}.jpg`));
    ...
    <div>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: title, pathname: { pathname: "/metiers/industrie/[slug]", params: { slug } } },
        ]}
      />
      ...
      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          {/* FAQ */}
          <div className="mt-12">
            <h2 className="font-serif text-2xl text-brand-black">{tr(INDUSTRIE_UI.faqTitle, locale)}</h2>

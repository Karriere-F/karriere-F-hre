...
import { IT_FICHES, IT_UI, tr } from "@/lib/it-metiers";
...
export async function generateMetadata({
  ...
  const fiche = IT_FICHES[slug];
  if (!fiche) return {};
  return buildMetadata({
    pathname: { pathname: "/metiers/informatique/[slug]", params: { slug } },
    locale,
...
export default async function ItFichePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <div>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: title, pathname: { pathname: "/metiers/informatique/[slug]", params: { slug } } },
        ]}
      />
      ...
      <PageHero
        ...
        image={imageExists ? `/images/metiers/informatique/${fiche.image}.jpg` : undefined}
        imageAlt={title}
      />
      ...
      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          {/* FAQ */}
          <div className="mt-12">
            <h2 className="font-serif text-2xl text-brand-black">{tr(IT_UI.faqTitle, locale)}</h2>

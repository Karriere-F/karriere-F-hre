...
import { LOGISTIQUE_FICHES, LOGISTIQUE_UI, tr } from "@/lib/logistique-metiers";
...
export async function generateMetadata({
  ...
  const fiche = LOGISTIQUE_FICHES[slug];
  if (!fiche) return {};
  return buildMetadata({
    pathname: { pathname: "/metiers/logistique/[slug]", params: { slug } },
    locale,
...
export default async function LogistiqueFichePage({
  ...
  const imageExists =
    !!fiche.image &&
    existsSync(join(process.cwd(), "public", "images", "metiers", "logistique", `${fiche.image}.jpg`));
    ...
    <div>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: title, pathname: { pathname: "/metiers/logistique/[slug]", params: { slug } } },
        ]}
      />
      ...
      <PageHero
        ...
        image={imageExists ? `/images/metiers/logistique/${fiche.image}.jpg` : undefined}
        imageAlt={title}
      />
      ...
      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          {/* FAQ */}
          <div className="mt-12">
            <h2 className="font-serif text-2xl text-brand-black">{tr(LOGISTIQUE_UI.faqTitle, locale)}</h2>

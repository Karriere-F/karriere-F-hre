...
import { CONSTRUCTION_FICHES, CONSTRUCTION_UI, tr } from "@/lib/construction-metiers";
...
export async function generateMetadata({
  ...
  const fiche = CONSTRUCTION_FICHES[slug];
  if (!fiche) return {};
  return buildMetadata({
    pathname: { pathname: "/metiers/construction/[slug]", params: { slug } },
    locale,
...
export default async function ConstructionFichePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const imageExists =
    !!fiche.image &&
    existsSync(join(process.cwd(), "public", "images", "metiers", "construction", `${fiche.image}.jpg`));
  ...
  return (
    <div>
      <BreadcrumbJsonLd
        locale={locale}
        items={[
          { name: tr(CONSTRUCTION_UI.home, locale), pathname: "/" },
          { name: tr(SECTEURS_UI.secteurs, locale), pathname: "/secteurs" },
          { name: tr(CONSTRUCTION_UI.sector, locale), pathname: "/secteurs/construction" },
          { name: title, pathname: { pathname: "/metiers/construction/[slug]", params: { slug } } },
        ]}
      />
      ...
      <PageHero
        ...
        image={imageExists ? `/images/metiers/construction/${fiche.image}.jpg` : undefined}
        imageAlt={title}
      />
      ...
      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          <div className="mt-10">
            ...
            >
              {tr(CONSTRUCTION_UI.apply, locale)}
          ...
          {/* FAQ */}
          <div className="mt-12">
            <h2 className="font-serif text-2xl text-brand-black">{tr(CONSTRUCTION_UI.faqTitle, locale)}</h2>

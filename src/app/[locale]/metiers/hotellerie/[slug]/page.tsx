...
import { HOTELLERIE_FICHES, HOTELLERIE_UI, tr } from "@/lib/hotellerie-metiers";
...
export async function generateMetadata({
  ...
  const fiche = HOTELLERIE_FICHES[slug];
  if (!fiche) return {};
  return buildMetadata({
    pathname: { pathname: "/metiers/hotellerie/[slug]", params: { slug } },
    locale,
...
export default async function HotellerieFichePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const imageExists =
    !!fiche.image &&
    existsSync(join(process.cwd(), "public", "images", "metiers", "hotellerie", `${fiche.image}.jpg`));
  ...
  return (
    <div>
      ...
      <PageHero
        ...
        image={imageExists ? `/images/metiers/hotellerie/${fiche.image}.jpg` : undefined}
        imageAlt={title}
      />
      ...
      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          {/* FAQ */}
          <div className="mt-12">
            <h2 className="font-serif text-2xl text-brand-black">{tr(HOTELLERIE_UI.faqTitle, locale)}</h2>

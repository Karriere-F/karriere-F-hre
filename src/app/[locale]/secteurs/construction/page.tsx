...
import { ArrowLeft, ArrowRight, GraduationCap, HardHat } from "lucide-react";
...
import { CONSTRUCTION_UI, tr } from "@/lib/construction-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/construction",
    locale,
    title: `${tr(CONSTRUCTION_UI.sector, locale)} : ${tr(SECTEURS_UI.chooseRouteTitle, locale)}`,
    description: tr(CONSTRUCTION_UI.hubIntro, locale),
  });
}

export default async function SecteursConstructionPage() {
  const locale = (await getLocale()) as Locale;

  const choices = [
    {
      href: "/secteurs/construction/ausbildung" as const,
      icon: GraduationCap,
      title: tr(SECTEURS_UI.routeAusbildungTitle, locale),
      intro: tr(SECTEURS_UI.routeAusbildungIntro, locale),
    },
    {
      href: "/secteurs/construction/diplome" as const,
      icon: HardHat,
  ...
  return (
    <div>
      ...
      <PageHero title={tr(CONSTRUCTION_UI.sector, locale)} subtitle={tr(SECTEURS_UI.chooseRouteTitle, locale)} />

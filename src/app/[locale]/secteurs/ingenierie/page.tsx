...
import { ArrowLeft, ArrowRight, PenTool, Wrench } from "lucide-react";
...
import { INGENIERIE_UI, tr } from "@/lib/ingenierie-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/ingenierie",
    locale,
    title: `${tr(INGENIERIE_UI.sector, locale)} : ${tr(SECTEURS_UI.chooseRouteTitle, locale)}`,
    description: tr(INGENIERIE_UI.hubIntro, locale),
  });
}

export default async function SecteursIngenieriePage() {
  const locale = (await getLocale()) as Locale;

  const choices = [
    {
      href: "/secteurs/ingenierie/ausbildung" as const,
      icon: PenTool,
      title: tr(SECTEURS_UI.routeAusbildungTitle, locale),
      intro: tr(SECTEURS_UI.routeAusbildungIntro, locale),
    },
    {
      href: "/secteurs/ingenierie/diplome" as const,
      icon: Wrench,
  ...
  return (
    <div>
      ...
      <PageHero title={tr(INGENIERIE_UI.sector, locale)} subtitle={tr(SECTEURS_UI.chooseRouteTitle, locale)} />

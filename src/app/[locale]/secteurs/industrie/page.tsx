...
import { ArrowLeft, ArrowRight, GraduationCap, Wrench } from "lucide-react";
...
import { INDUSTRIE_UI, tr } from "@/lib/industrie-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/industrie",
    locale,
    title: `${tr(INDUSTRIE_UI.sector, locale)} : ${tr(SECTEURS_UI.chooseRouteTitle, locale)}`,
    description: tr(INDUSTRIE_UI.hubIntro, locale),
  });
}

export default async function SecteursIndustriePage() {
  const locale = (await getLocale()) as Locale;

  const choices = [
    {
      href: "/secteurs/industrie/ausbildung" as const,
      icon: GraduationCap,
      title: tr(SECTEURS_UI.routeAusbildungTitle, locale),
      intro: tr(SECTEURS_UI.routeAusbildungIntro, locale),
    },
    {
      href: "/secteurs/industrie/diplome" as const,
      icon: Wrench,
  ...
  return (
    <div>
      <BreadcrumbJsonLd
        locale={locale}
        items={[
          { name: tr(INDUSTRIE_UI.home, locale), pathname: "/" },
          { name: tr(SECTEURS_UI.secteurs, locale), pathname: "/secteurs" },
          { name: tr(INDUSTRIE_UI.sector, locale), pathname: "/secteurs/industrie" },
        ]}
      />

      <PageHero title={tr(INDUSTRIE_UI.sector, locale)} subtitle={tr(SECTEURS_UI.chooseRouteTitle, locale)} />

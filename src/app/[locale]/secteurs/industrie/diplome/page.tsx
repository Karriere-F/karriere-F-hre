...
import { INDUSTRIE_DIPLOME_METIERS, INDUSTRIE_FICHES, INDUSTRIE_UI, tr } from "@/lib/industrie-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/industrie/diplome",
    locale,
    title: `${tr(INDUSTRIE_UI.sector, locale)} : ${tr(SECTEURS_UI.routeDiplomeTitle, locale)}`,
    description: tr(SECTEURS_UI.routeDiplomeIntro, locale),
  });
}

export default async function IndustrieDiplomePage() {
  const locale = (await getLocale()) as Locale;
  return (
    <div>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: tr(SECTEURS_UI.routeDiplomeTitle, locale), pathname: "/secteurs/industrie/diplome" },
        ]}
      />

      <PageHero title={tr(INDUSTRIE_UI.hubTitle, locale)} subtitle={tr(SECTEURS_UI.routeDiplomeIntro, locale)} />
      ...
      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          ...
          <div className="mt-6">
            <MetierList
              metiers={INDUSTRIE_DIPLOME_METIERS}
              locale={locale}

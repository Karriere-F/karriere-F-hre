...
import { IT_AUSBILDUNG_METIERS, IT_FICHES, IT_UI, tr } from "@/lib/it-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/informatique/ausbildung",
    locale,
    title: `${tr(IT_UI.sector, locale)} : ${tr(SECTEURS_UI.routeAusbildungTitle, locale)}`,
    description: tr(SECTEURS_UI.routeAusbildungIntro, locale),
  });
}

export default async function ItAusbildungPage() {
  const locale = (await getLocale()) as Locale;
  return (
    <div>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: tr(SECTEURS_UI.routeAusbildungTitle, locale), pathname: "/secteurs/informatique/ausbildung" },
        ]}
      />

      <PageHero title={tr(IT_UI.hubTitle, locale)} subtitle={tr(SECTEURS_UI.routeAusbildungIntro, locale)} />

      <section className="bg-brand-white">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-16 sm:py-20">
          <Link
            href="/secteurs/informatique"

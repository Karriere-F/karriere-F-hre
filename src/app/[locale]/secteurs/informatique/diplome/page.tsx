...
import { IT_DIPLOME_METIERS, IT_FICHES, IT_UI, tr } from "@/lib/it-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/informatique/diplome",
    locale,
    title: `${tr(IT_UI.sector, locale)} : ${tr(SECTEURS_UI.routeDiplomeTitle, locale)}`,
    description: tr(SECTEURS_UI.routeDiplomeIntro, locale),
  });
}

export default async function ItDiplomePage() {
  const locale = (await getLocale()) as Locale;
  return (
    <div>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { name: tr(SECTEURS_UI.routeDiplomeTitle, locale), pathname: "/secteurs/informatique/diplome" },
        ]}
      />

      <PageHero title={tr(IT_UI.hubTitle, locale)} subtitle={tr(SECTEURS_UI.routeDiplomeIntro, locale)} />

...
import { ArrowLeft, ArrowRight, GraduationCap, UtensilsCrossed } from "lucide-react";
...
import { HOTELLERIE_UI, tr } from "@/lib/hotellerie-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/hotellerie",
    locale,
    title: `${tr(HOTELLERIE_UI.sector, locale)} : ${tr(SECTEURS_UI.chooseRouteTitle, locale)}`,
    description: tr(HOTELLERIE_UI.hubIntro, locale),
  });
}

export default async function SecteursHotelleriePage() {
  const locale = (await getLocale()) as Locale;

  const choices = [
    {
      href: "/secteurs/hotellerie/ausbildung" as const,
      icon: GraduationCap,
      title: tr(SECTEURS_UI.routeAusbildungTitle, locale),
      intro: tr(SECTEURS_UI.routeAusbildungIntro, locale),
    },
    {
      href: "/secteurs/hotellerie/diplome" as const,
  ...
  return (
    <div>
      ...
      <PageHero title={tr(HOTELLERIE_UI.sector, locale)} subtitle={tr(SECTEURS_UI.chooseRouteTitle, locale)} />

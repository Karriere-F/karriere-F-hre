...
import { ArrowLeft, ArrowRight, GraduationCap, Truck } from "lucide-react";
...
import { LOGISTIQUE_UI, tr } from "@/lib/logistique-metiers";

export async function generateMetadata(): Promise<Metadata> {
  const locale = (await getLocale()) as Locale;
  return buildMetadata({
    pathname: "/secteurs/logistique",
    locale,
    title: `${tr(LOGISTIQUE_UI.sector, locale)} : ${tr(SECTEURS_UI.chooseRouteTitle, locale)}`,
    description: tr(LOGISTIQUE_UI.hubIntro, locale),
  });
}

export default async function SecteursLogistiquePage() {
  const locale = (await getLocale()) as Locale;

  const choices = [
    {
      href: "/secteurs/logistique/ausbildung" as const,
      icon: GraduationCap,
      title: tr(SECTEURS_UI.routeAusbildungTitle, locale),
      intro: tr(SECTEURS_UI.routeAusbildungIntro, locale),
    },
    {
      href: "/secteurs/logistique/diplome" as const,
      icon: Truck,
  ...
  return (
    <div>
      <BreadcrumbJsonLd
        locale={locale}
        items={[
          { name: tr(LOGISTIQUE_UI.home, locale), pathname: "/" },
          { name: tr(SECTEURS_UI.secteurs, locale), pathname: "/secteurs" },
          { name: tr(LOGISTIQUE_UI.sector, locale), pathname: "/secteurs/logistique" },
        ]}
      />

      <PageHero title={tr(LOGISTIQUE_UI.sector, locale)} subtitle={tr(SECTEURS_UI.chooseRouteTitle, locale)} />
